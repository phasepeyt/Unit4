...
while True:
    user_choice = input("Choose 1 = update item, 2 = view item, 3 = view list, 4 = combine list, 5 = count number of qtips in a list, 6 = reverse list, or q = quit: ")
    ...
    def countqtips(list_num):
        count = 0
        for item in shopping_lists[list_num]:
            if item == "q-tips":
                count += 1
        print(f"{count} q-tips")

    def drinkmoremilk():
        for list in shopping_lists:
            if 'milk' not in list:
                shopping_lists.append('milk' + ', ' + 'cookies')
    ...
    if user_choice == '1':
        list_choice = int(input("which list would u like to update? ")) -1
        ...
        list_choice = int(input("which list would u like to view? ")) -1
        ...
        list_choice = int(input("which list would u like to count the qtips in? ")) - 1

        countqtips(list_choice)

    elif user_choice == '6':
        shopping_cart = shopping_lists[::-1]
        print(shopping_cart)
